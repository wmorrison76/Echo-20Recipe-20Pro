 ...
 * Check system health for Echo, OpenAI, and Pinecone
 */
router.get("/status", async (_req: Request, res: Response) => {
  const status: SystemHealthStatus = {
    ...
    services: {
      echo: {
        status: "operational",
        message: "Echo AI system is ready",
      },
      openai: {
        status: "error",
  ...
  if (!openaiKey) {
    ...
    status.services.openai.message = "OPENAI_API_KEY not configured";
    ...
    try {
      ...
      if (response.ok) {
        status.services.openai.status = "operational";
        status.services.openai.message = "OpenAI API is accessible";
        status.services.openai.model = "gpt-4-turbo-preview";
        ...
        status.services.openai.status = "warning";
        status.services.openai.message = `OpenAI API returned status ${response.status}`;
      }
    } catch (error: any) {
      status.services.openai.status = "error";
      status.services.openai.message = `OpenAI API error: ${error.message}`;
      status.healthy = false;
    }
  }
  ...
  if (!pineconeKey) {
    status.services.pinecone.status = "error";
    status.services.pinecone.message = "PINECONE_API_KEY not configured";
    status.healthy = false;
  } else {
    try {
      ...
      if (response.ok) {
        ...
        const echoIndex = data.indexes?.find(
          (idx: any) => idx.name === "echo-knowledge",
        );

        if (echoIndex) {
          status.services.pinecone.status = "operational";
          status.services.pinecone.message =
            "Pinecone is connected and echo-knowledge index is ready";
          status.services.pinecone.indexName = "echo-knowledge";
        } else {
          status.services.pinecone.status = "warning";
          status.services.pinecone.message =
            "Pinecone is connected but echo-knowledge index not found. Will create on first knowledge store.";
        }
      } else if (response.status === 401) {
        status.services.pinecone.status = "error";
        status.services.pinecone.message =
        ...
        status.services.pinecone.status = "warning";
        status.services.pinecone.message = `Pinecone returned status ${response.status}`;
      }
    } catch (error: any) {
      status.services.pinecone.status = "error";
      status.services.pinecone.message = `Pinecone error: ${error.message}`;
      status.healthy = false;
    }
  }
 ...
 * Quick check if system is ready to start training
 */
...
router.post("/verify", async (_req: Request, res: Response) => {
  ...
  // Check 2: OpenAI API is accessible
  if (openaiKey) {
    try {
  ...
  // Check 3: Pinecone API Key exists
  const pineconeKey = process.env.PINECONE_API_KEY;
  verification.checks.push({
    name: "Pinecone API Key",
    passed: !!pineconeKey,
    error: pineconeKey ? undefined : "PINECONE_API_KEY not configured",
  });
  ...
  if (pineconeKey) {
    try {
      ...
      // Check 5: Echo knowledge index exists
      if (response.ok) {
        const data = (await response.json()) as any;
        const echoIndex = data.indexes?.find(
          (idx: any) => idx.name === "echo-knowledge",
        ...
        verification.checks.push({
          name: "Echo Knowledge Index",
          passed: !!echoIndex,
          error: echoIndex ? undefined : "echo-knowledge index not found",
          details: {
            indexFound: !!echoIndex,
            indexName: echoIndex?.name || "N/A",
            dimension: echoIndex?.dimension || "N/A",
            metric: echoIndex?.metric || "N/A",
          },
        });
      }
    } catch (error: any) {
  ...
  // Check 6: Echo Training Routes
  verification.checks.push({
    name: "Echo Training Routes",
    passed: true,
    details: {
      routes: [
  ...
  const allPassed = verification.checks.every((check) => check.passed);
  const httpStatus = allPassed ? 200 : 503;

  res.status(httpStatus).json({
    ...verification,
    summary: {
      allChecks: verification.checks.length,
      passed: verification.checks.filter((c) => c.passed).length,
      failed: verification.checks.filter((c) => !c.passed).length,
