...
router.post("/api/recipes/deployments", async (req: Request, res: Response) => {
  try {
    ...
    // Validate input
    if (!deployment_name || !packets || packets.length === 0) {
      return res.status(400).json({
        error: "Missing required fields: deployment_name and packets",
      });
    }

    if (!all_outlets && (!target_outlets || target_outlets.length === 0)) {
      return res.status(400).json({
        error: "Must specify either all_outlets or target_outlets",
      });
    }
    ...
    const { data: deployment, error: deploymentError } = await supabase
      ...
      .insert({
        ...
        deployment_name,
        description,
        target_outlets: all_outlets ? null : target_outlets,
        target_locations: target_locations || [],
        all_outlets: all_outlets || false,
        deployment_type,
        ...
        rollback_deadline: rollback_deadline
          ? new Date(rollback_deadline).toISOString()
          : null,
    ...
    if (deploymentError) {
      return res.status(500).json({ error: deploymentError.message });
    }

    // Create deployment packets
    const packetInserts = packets.map((packet) => ({
      deployment_id: deployment.id,
      recipe_id: packet.recipe_id,
      recipe_name: packet.recipe_name,
      packet_data: packet.packet_data,
      previous_recipe_version_hash: packet.previous_recipe_version_hash,
      new_recipe_version_hash: hashObject(packet.packet_data),
      changes_summary: packet.changes_summary || {},
    }));

    const { error: packetsError } = await supabase
      .from("deployment_packets")
      .insert(packetInserts);

    if (packetsError) {
      return res.status(500).json({ error: packetsError.message });
    }

    // Create store confirmations for all target outlets
    const outletIds = all_outlets
      ? target_outlets // If all_outlets, this should be populated elsewhere
      : target_outlets;

    const confirmationInserts = outletIds.map((outletId: string) => ({
      deployment_id: deployment.id,
      outlet_id: outletId,
      location_id: null,
      organization_id: organizationId,
    ...
    if (confirmationError) {
      return res.status(500).json({ error: confirmationError.message });
    }
    ...
    res.json({
      ...
      deployment: {
        ...
        store_confirmations_pending: outletIds.length,
      },
    });
  } catch (error) {
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const confirmationData = req.body as ConfirmationRequest;
      ...
      const { data: confirmation, error: updateError } = await supabase
        ...
        .update({
          ...
          confirmed_by_user_id: confirmationData.confirmed_by_user_id,
          confirmed_by_username: confirmationData.confirmed_by_username,
          rejection_reason: confirmationData.rejection_reason,
          failure_reason: confirmationData.failure_reason,
          received_at:
            confirmationData.status === "received" ? new Date() : undefined,
          confirmed_at:
            confirmationData.status === "confirmed" ? new Date() : undefined,
          applied_at:
            confirmationData.status === "applied" ? new Date() : undefined,
        })
      ...
      const statusMap: Record<string, string> = {
        received: "store_received",
        confirmed: "store_confirmed",
        applied: "store_applied",
      ...
      await logDeploymentActivity(
        deploymentId,
        statusMap[confirmationData.status] || "store_confirmation_updated",
        (req as any).user?.id,
        organizationId,
        {
          outlet_id: outletId,
...
// Cancel a deployment
router.post(
  "/api/recipes/deployments/:id/cancel",
  async (req: Request, res: Response) => {
    try {
      if (!supabase) {
        return res.status(503).json({ error: "Database service unavailable" });
