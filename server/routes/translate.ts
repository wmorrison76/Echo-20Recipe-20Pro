...
export default async function translate(req: Request, res: Response) {
  try {
    const { recipes, targetLanguage } = req.body;
    ...
    const translatedRecipes = await Promise.all(
      recipes.map(async (recipe: any, index: number) => {
        try {
          ...
          const prompt = `Translate the following recipe to ${targetLanguageName}. Keep the structure and return valid JSON.
...
Return ONLY valid JSON with the same structure but all text translated to ${targetLanguageName}. Do not include any markdown or extra text.`;
              ...
              messages: [
                {
                  role: "system",
                  content:
                    "You are a professional translator. Translate recipe content accurately, maintaining all technical culinary terms. Return only valid JSON.",
                },
                {
                  role: "user",
                  content: prompt,
          ...
          console.error(`Error translating recipe ${index}:`, error);
          return recipe;
        }
      }),
    );

    res.json({ recipes: translatedRecipes });
  } catch (error) {
