...
async function conductAutonomousExchange(
  ...
  const knowledge = await extractKnowledgeFromResponse(openaiResponse, profile);
...
Focus on expertise and mastery. ${progressText}`;
    ...
    const focusArea =
      profile.focusAreas[exchangeNumber % profile.focusAreas.length];
...
async function getOpenAIResponse(prompt: string): Promise<string | null> {
  ...
  try {
    ...
    console.error(
      "[MultiDomainTraining] Failed to get OpenAI response:",
      error,
    );
    return null;
  }
...
Return ONLY valid JSON array.`;
    ...
    console.error("[MultiDomainTraining] Knowledge extraction failed:", error);
    return [];
  }
}
...
router.post("/record-completed", async (_req: Request, res: Response) => {
  try {
    ...
    console.log(
      "[MultiDomainTraining] Successfully recorded completed domains",
    );
...
router.get("/sessions/completed", async (_req: Request, res: Response) => {
  try {
    ...
    console.error(
      "[MultiDomainTraining] Get completed sessions failed:",
      error,
    );
    return res.status(500).json({
      success: false,
 ...
 * Store training vectors to Pinecone from actual training sessions
 * Accepts training data as input instead of using hardcoded data
 */
router.post(
  "/pinecone/store-completed",
  async (req: Request, res: Response) => {
    try {
      const { trainingData, sessionId } = req.body as {
        trainingData?: Array<{ profileId: string; domain: string; title: string; content: string; confidence: number }>;
        sessionId?: string;
      };
      ...
      if (!trainingData || trainingData.length === 0) {
        return res.json({
          success: false,
          stored: 0,
          message: "No training data provided. POST with trainingData array in body.",
          error: "EMPTY_TRAINING_DATA",
        });
      }

      const finalSessionId = sessionId || `training-session-${Date.now()}-${Math.random().toString(36).substring(7)}`;

      console.log(
        `[MultiDomainTraining] Storing ${trainingData.length} training vectors to Pinecone (sessionId: ${finalSessionId})`,
      );

      const result = await storeTrainingDataToPinecone(finalSessionId, trainingData);
      ...
      return res.json({
        ...
        message: result.success
          ? `Successfully stored ${result.stored}/${trainingData.length} training vectors to Pinecone`
