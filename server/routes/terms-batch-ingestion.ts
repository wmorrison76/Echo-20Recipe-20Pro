/**
 * Terms Batch Ingestion API
 * Handles bulk term ingestion for Phase 3 production deployment
 * Coordinates deduplication, embedding generation, and dual storage
 */
 ...
 * }
 */
termsBatchIngestionRouter.post(
  "/ingest-batch",
  async (req: Request, res: Response) => {
    const request = req.body as BatchIngestRequest;

    if (!request.terms || !Array.isArray(request.terms)) {
      return res.status(400).json({
        success: false,
        error: "Request must include 'terms' array",
      });
    }
    ...
    const batchSource = request.source || "unknown";
    const batchIndex = request.batchIndex ?? -1;

    console.log(
      `[TermsIngestion] Batch ${batchIndex} (${batchSource}): Received ${request.terms.length} terms`,
    );
    ...
    try {
      ...
      const stats = await ingestionOrchestrator.ingestTerms(request.terms);
      ...
      const response: BatchIngestResponse = {
        ...
        stats: {
          ...
          storageSuccessful: stats.storageSuccessful,
      ...
      console.error(`[TermsIngestion] Batch ${batchIndex} failed:`, message);
...
/**
 * POST /api/terms/ingest-dedup-only
 * Test deduplication without embedding generation
 * Useful for pre-processing validation
 */
termsBatchIngestionRouter.post(
  "/ingest-dedup-only",
  async (req: Request, res: Response) => {
    ...
    try {
      const startTime = Date.now();
      const result = deduplicationService.deduplicate(request.terms);
      const duration = Date.now() - startTime;
      ...
      res.json({
        ...
        duration,
        message: `Deduplication complete: ${request.terms.length} → ${result.unique.length} terms (${result.deduplicationRate.toFixed(1)}% dedup rate)`,
      });
    } catch (error) {
      res.status(500).json({
        success: false,
