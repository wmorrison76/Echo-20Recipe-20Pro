...
/**
 * Generate synthetic terms for testing
 */
function generateSyntheticTerms(count: number): TermForDedup[] {
  const categories = [
    "technique",
    "ingredient",
    "equipment",
  ...
  for (let i = 0; i < count; i++) {
    ...
    terms.push({
      term: `${adj} ${noun} ${i}`,
      definition: `A ${adj} culinary ${noun} used in professional kitchens. This is term number ${i} in the test set. Includes related techniques, applications, and industry best practices.`,
      categories: [category],
      masteryLevel: ["fundamental", "intermediate", "advanced", "expert"][
        Math.floor(Math.random() * 4)
      ],
      confidence: 0.75 + Math.random() * 0.25,
      sourceType: "test-generated",
      metadata: {
        testIndex: i,
 ...
 * Test deduplication performance with synthetic duplicates
 */
loadTestingRouter.post(
  "/load-test/deduplication",
  async (req: Request, res: Response) => {
    try {
      console.log("[LoadTest] Starting deduplication test");

      const { termCount = 5000, duplicateRate = 0.2 } = req.body;

      // Generate terms with intentional duplicates
      const baseTerms = generateSyntheticTerms(
        Math.floor(termCount * (1 - duplicateRate)),
      );
      const duplicates: TermForDedup[] = [];
      ...
      const allTerms = [...baseTerms, ...duplicates];

      const startTime = Date.now();
      const result = deduplicationService.deduplicate(allTerms);
      ...
      res.json({
        ...
        test: "deduplication",
        duration,
        input: allTerms.length,
        output: result.unique.length,
        deduplicationRate: result.deduplicationRate,
        duplicatesFound: result.duplicates.size,
        metricsPerTerm: {
          averageTimeMs: duration / allTerms.length,
        },
      });
    } catch (error) {
      console.error("[LoadTest] Deduplication test error:", error);
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : "Unknown error",
      });
...
/**
 * GET /api/load-test/estimated-time
 * Estimate time needed for 180K term ingestion based on current performance
 */
loadTestingRouter.get(
  "/load-test/estimated-time",
  async (req: Request, res: Response) => {
    try {
      // Run a quick 1000-term test to measure performance
      const quickTerms = generateSyntheticTerms(1000);
      const startTime = Date.now();

      const result = await ingestionOrchestrator.ingestTerms(quickTerms);

      const duration = Date.now() - startTime;
      const timePerTerm = duration / result.afterDedup;
      const estimatedTotal180K = timePerTerm * 180000; // After dedup, expecting ~180K → ~150K

      res.json({
        success: true,
        quickTestSize: 1000,
        quickTestDuration: duration,
        estimations: {
          seconds: estimatedTotal180K / 1000,
          minutes: estimatedTotal180K / 60000,
          hours: estimatedTotal180K / 3600000,
          humanReadable: formatDuration(estimatedTotal180K),
        },
        assumptions: {
          inputTerms: 180000,
          expectedAfterDedup: "~150000 (15-20% deduplication)",
          concurrentWorkers: 5,
          batchSizeEmbeddings: 100,
        },
      });
    } catch (error) {
      console.error("[LoadTest] Estimation error:", error);
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : "Unknown error",
      });
