    ...
    for (let i = 0; i < terms.length; i++) {
      ...
      try {
        ...
        const standardizedTerm = {
          ...
          usage: {
            primary: definition.substring(0, 80),
            secondary: [],
            context: `${categoryDisplay} knowledge`,
          },
          categories: [categoryDisplay, "culinary"],
          applications: {
            primary: `Advanced culinary knowledge in ${categoryDisplay}`,
            examples: [],
            contexts: [`${categoryDisplay} techniques`],
          },
          relatedTerms: termData.related_terms || [],
          history: {
            period: `Advanced culinary science`,
