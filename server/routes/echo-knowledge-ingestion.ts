/**
 * Echo Knowledge Ingestion API Routes
 * Endpoints for ingesting knowledge sources into internal pgvector storage
 */
...
} from "../lib/knowledge-ingestion-service";
 ...
 * Ingest all knowledge sources (Master Dictionary + Pinecone)
 */
router.post("/knowledge/ingest/all", async (req: Request, res: Response) => {
  try {
    const progress = ingestionController.getProgress();
    if (progress.status === "in_progress") {
      return res.status(409).json({
    ...
    console.log("[API] Starting bulk knowledge ingestion...");

    res.json({
      status: "started",
      message: "Bulk knowledge ingestion started",
      progress: ingestionController.getProgress(),
    });

    // Run ingestion in background
    ingestionController.ingestAll().catch((error) => {
      console.error("[API] Bulk knowledge ingestion failed:", error);
    });
  } catch (error) {
    console.error("[API] Error starting bulk knowledge ingestion:", error);
    res.status(500).json({
      status: "error",
      message: "Failed to start bulk knowledge ingestion",
      error: error instanceof Error ? error.message : "Unknown error",
    });
  }
});
 ...
 */
router.get("/knowledge/ingest/status", async (req: Request, res: Response) => {
  try {
    const progress = ingestionController.getProgress();
    const stats = await ingestionController.getStatistics();
