...
function getSupabaseClient(): SupabaseClient {
  if (!supabaseClient) {
    ...
    if (!supabaseUrl || !supabaseKey) {
      console.warn("[Inventory Operations] Supabase credentials not configured");
...
// GET /api/inventory/items - Search inventory items with fuzzy match
router.get("/items", async (req: Request, res: Response) => {
  try {
    ...
    const results = items
      .map((item) => ({
        ...item,
        score: fuzzyMatch(searchText, item.name),
      }))
  ...
  try {
    ...
    const { data: transfer, error } = await supabase
      ...
      .insert({
        ...
        quantity,
        unit: unit || "unit",
        from_department: from_department || "kitchen",
        to_department,
        requested_by,
        notes,
        status: "pending",
...
// GET /api/inventory/transfers/:id - Get specific transfer
...
// PUT /api/inventory/transfers/:id - Update transfer status
...
// GET /api/inventory/recalls - Get active food recalls
...
// GET /api/inventory/recalls/:id - Get specific recall
