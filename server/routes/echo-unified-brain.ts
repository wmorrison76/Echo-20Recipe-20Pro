 ...
 * Main endpoint for querying the Unified Brain across all domains
 */
...
router.post("/batch", async (req: Request, res: Response) => {
  try {
    ...
    if (!Array.isArray(requests)) {
      return res.status(400).json({
        error: "Request body must contain 'requests' array",
      });
    }
...
/**
 * GET /api/echo-unified/capabilities
 * List all available query types and their requirements
 */
