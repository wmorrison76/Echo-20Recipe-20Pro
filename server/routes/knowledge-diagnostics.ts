/**
 * Knowledge Diagnostics API Routes
 * Provides detailed information about all knowledge sources and storage status
 */
 ...
 * Get comprehensive knowledge system diagnostics
 */
router.get("/diagnostics", async (req: Request, res: Response) => {
  try {
    ...
    const diagnostics = {
      ...
      masterDictionary: {
        totalTerms: masterDictStats.totalTerms,
      ...
      summary: {
        readiness: calculateReadiness(internalStats, masterDictStats),
        nextSteps: getNextSteps(internalStats, masterDictStats),
      },
    };
    ...
    console.error("[Diagnostics] Error getting diagnostics:", error);
    res.status(500).json({
      status: "error",
      message: "Failed to get knowledge diagnostics",
...
router.get("/status", async (req: Request, res: Response) => {
  try {
    ...
    console.error("[Diagnostics] Error getting status:", error);
    res.status(500).json({
      status: "error",
      message: "Failed to get knowledge status",
      error: error instanceof Error ? error.message : "Unknown error",
    });
  }
});
 ...
 * Verify all knowledge sources and check health
 */
router.post("/verify", async (req: Request, res: Response) => {
  try {
    console.log("[Diagnostics] Starting knowledge verification...");

    const checks = {
      masterDictionary: await verifyMasterDictionary(),
      internalStorage: await verifyInternalStorage(),
    ...
    console.error("[Diagnostics] Error during verification:", error);
    res.status(500).json({
      status: "error",
...
function calculateReadiness(internalStats: any, masterDictStats: any): string {
  const internalVectors = internalStats.total || 0;
  const masterTerms = masterDictStats.totalTerms || 0;

  if (internalVectors > 1000 && masterTerms > 5000) {
    return "EXCELLENT - All sources loaded";
  } else if (internalVectors > 100 && masterTerms > 1000) {
    return "GOOD - Core knowledge available";
  } else if (masterTerms > 1000) {
    return "READY - Master Dictionary loaded, internal storage needs configuration";
  } else if (internalVectors > 0) {
    return "PARTIAL - Only internal storage active";
  } else {
    return "NEEDS_SETUP - No knowledge sources loaded";
  }
}

function getNextSteps(internalStats: any, masterDictStats: any): string[] {
  const steps: string[] = [];
  const internalVectors = internalStats.total || 0;
  const masterTerms = masterDictStats.totalTerms || 0;
...
async function verifyMasterDictionary(): Promise<{
  ...
  try {
    ...
    return {
      healthy: stats.totalTerms > 0,
      terms: stats.totalTerms,
      message: `Master Dictionary loaded with ${stats.totalTerms} terms`,
    };
  } catch (error) {
    return {
      healthy: false,
      terms: 0,
      message: `Error loading Master Dictionary: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}
...
async function verifyInternalStorage(): Promise<{
  ...
  try {
    ...
    return {
      ...
      message: healthy
        ? stats.total > 0
          ? `Internal storage configured with ${stats.total} vectors`
          : "Supabase not configured (use internal storage only)"
        : "Error accessing internal storage",
    };
  } catch (error) {
    return {
      healthy: false,
      vectors: 0,
      message: `Error accessing internal storage: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}
  ...
  try {
    ...
    if (!pineconeKey) {
      return {
        healthy: true,
        available: false,
        message: "Pinecone API key not configured (optional)",
      };
    }

    // Quick verification by checking if we can import the client
    try {
      const { Pinecone } = await import("@pinecone-database/pinecone");
      return {
        healthy: true,
        available: true,
        message: "Pinecone configured and available",
      };
    } catch (error) {
      return {
        healthy: false,
        available: false,
        message: `Pinecone configured but unavailable: ${error instanceof Error ? error.message : String(error)}`,
      };
    }
  } catch (error) {
    return {
      healthy: false,
      available: false,
      message: `Error checking Pinecone: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}
...
function generateRecommendations(checks: any): string[] {
  ...
  if (!checks.masterDictionary.healthy) {
    recommendations.push("⚠️ Master Dictionary is not loading properly");
  }
  ...
  if (recommendations.length === 0) {
    recommendations.push("✓ All knowledge sources are healthy and configured");
  }
