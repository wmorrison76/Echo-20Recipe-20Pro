/**
 * Pinecone Verification Service
 * Verifies that training data has been stored in Pinecone
 */
...
async function getPineconeModule() {
  try {
    ...
    return module.Pinecone;
  } catch (error) {
    console.error("[PineconeVerification] Failed to import Pinecone:", error);
    return null;
  }
}
...
async function getPineconeClient() {
  if (!pineconeClientInstance && PINECONE_API_KEY) {
    ...
    if (!Pinecone) {
      console.warn("[PineconeVerification] Pinecone module not available");
      return null;
    }
 ...
 */
export async function getPineconeStatus(): Promise<PineconeStatus> {
  const client = await getPineconeClient();

  if (!client || !PINECONE_API_KEY) {
    return {
      connected: false,
      indexName: KNOWLEDGE_INDEX,
      indexStats: null,
      trainingDataVectors: null,
      error: "Pinecone not configured or client unavailable",
    };
  }
  ...
  try {
    ...
    // Count vectors by domain (look for vectors with training-related tags)
    const trainingStats: Record<string, number> = {};
    let totalTrainingVectors = 0;

    // Query for training vectors if possible
    try {
      // Create a dummy embedding to search for training data
      const dummyEmbedding = new Array(1536).fill(0);

      // Query with a filter for sourceType: "user_trained"
      const results = await index.query({
        vector: dummyEmbedding,
        topK: 10000, // Get as many as possible in one query
        includeMetadata: true,
        filter: {
          sourceType: { $eq: "user_trained" },
        },
      });
      ...
      console.warn(
        "[PineconeVerification] Could not query training vectors:",
        queryError.message,
      );
    }
    ...
    return {
      ...
      trainingDataVectors: {
        total: totalTrainingVectors,
        byDomain: trainingStats,
      },
    };
  } catch (error: any) {
    ...
    return {
      ...
      indexStats: null,
      trainingDataVectors: null,
      error: error.message || "Failed to get Pinecone status",
    };
  }
}
 ...
 */
export async function verifyTrainingVectors(): Promise<{
  found: boolean;
  count: number;
  vectors: Array<{
    id: string;
  ...
  try {
    ...
    // Query for all training vectors
    const results = await index.query({
      vector: dummyEmbedding,
      topK: 10000,
      includeMetadata: true,
    ...
    const vectors = (results.matches || []).map((match: any) => ({
      id: match.id,
      domain: match.metadata?.domain || "unknown",
      title: match.metadata?.title || "unknown",
      confidence: match.metadata?.confidence || 0,
    }));

    return {
      found: vectors.length > 0,
      count: vectors.length,
      vectors,
    };
  } catch (error: any) {
    console.error("[PineconeVerification] Error verifying vectors:", error);
    return {
      found: false,
      count: 0,
      vectors: [],
...
export async function storeTrainingDataToPinecone(
  ...
  try {
      ...
      try {
        ...
        vectors.push({
          ...
          metadata: {
            ...
            tags: [item.profileId, item.domain],
    ...
    return {
      success: false,
      stored: 0,
      error: error.message || "Failed to store training data",
    };
  }
}
