 ...
 * Handles image-based PDFs that regular text extraction can't process
 */
...
export interface OCRResult {
  text: string;
  confidence: number; // Average confidence from OCR
  pages: OCRPageResult[];
  totalPages: number;
  extractedAt: Date;
}
...
export class OCRService {
  ...
  async terminate(): Promise<void> {
    if (this.tesseractWorker) {
      ...
      console.log("[OCR] Tesseract terminated");
    }
  }
  ...
  async extractFromPDF(
    ...
    console.log(`[OCR] Starting OCR extraction from ${fileName}`);

    try {
      // Parse PDF to get page data
      ...
      // Process each page (limit to first 100 pages for reasonable processing time)
      const pagesToProcess = Math.min(totalPages, 100);
  ...
  private async ocrPage(
    ...
    try {
      ...
      return {
        ...
        confidence: (result.data.confidence || 0) / 100, // Convert from 0-100 to 0-1
        language: "eng",
      };
    } catch (error) {
      console.error(`[OCR] Recognition failed for page ${pageNumber}:`, error);
      return {
        pageNumber,
        text: "",
        confidence: 0,
  ...
  private pageToImage(data: any): Promise<string> {
    return new Promise((resolve) => {
      // This would use pdf-render or similar to convert page to image
  ...
  /**
   * Extract text from image directly (for pre-rendered images)
   */
  async extractFromImage(imageBuffer: Buffer): Promise<string> {
    if (!this.initialized) {
      await this.initialize();
    }
  ...
  async extractFromImages(
    ...
    for (let i = 0; i < imageBuffers.length; i++) {
      try {
        const text = await this.extractFromImage(imageBuffers[i]);
        results.push(text);
        ...
        console.warn(`[OCR] Failed to process image ${i}:`, error);
        results.push("");
      }
    }
