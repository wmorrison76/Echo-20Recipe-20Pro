...
Your role is to design rigorous, executable food experiments that:
...
- Account for ingredient seasonality and availability
...
Return a complete experiment design as JSON with this exact structure (no markdown, no code blocks):
{
  ...
  "variables": [
    {
      ...
      "unit": "unit",
      "importance": "critical|important|nice-to-have",
      "rationale": "Why this variable matters"
    }
  ],
  "controls": {
    "baselineIngredients": ["list", "of", "ingredients"],
    "standardProcedure": "Step-by-step control procedure",
    "controlDescription": "What the control establishes"
  },
  "testMatrix": {
    "sampleSize": number (minimum 3),
    "replicates": number,
  ...
  "successCriteria": [
    {
      ...
      "how_measured": "Method of measurement",
  ...
  "estimatedTimeline": {
    ...
    "critical_path": "Description of longest dependency chain"
  },
  ...
  try {
    ...
    const design = JSON.parse(text) as ExperimentDesignResponse;

    if (!design.hypothesis || !design.variables || !design.controls) {
...
Experiment: "${request.experimentTitle || "Unnamed"}"
...
Return ONLY valid JSON (no markdown, no code blocks):
{
  ...
  try {
    const message = await client.messages.create({
      ...
      max_tokens: 2048,
      system:
        "You are a statistical analyst and data quality expert. Evaluate experimental data for validity and reliability. Respond with ONLY valid JSON.",
      messages: [
        {
          role: "user",
          content: userPrompt,
    ...
    const text = content.text.trim();
    const response = JSON.parse(text) as ValidationResponse;
    return response;
  } catch (error) {
...
Base Procedure: ${request.procedure}

Create a production-ready SOP with:
...
Return ONLY valid JSON (no markdown):
{
  ...
  "procedure": [
    {
      "step": 1,
      "description": "detailed procedure step",
  ...
  "allergenWarning": "FDA allergen statement"
}`;
  ...
  try {
    const message = await client.messages.create({
      ...
      max_tokens: 3000,
      system:
        "You are an expert culinary technologist creating professional SOPs for food production. Be precise, detailed, and safety-focused. Respond with ONLY valid JSON.",
      messages: [
        {
          role: "user",
          content: userPrompt,
    ...
    const text = content.text.trim();
    const sop = JSON.parse(text) as SOPResponse;
    return sop;
  } catch (error) {
...
- Equipment/methods from similar experiments
...
["recommendation 1", "recommendation 2", "recommendation 3"]`;
  ...
  try {
    const message = await client.messages.create({
      ...
      max_tokens: 1024,
      system:
        "You are an expert culinary researcher providing recommendations. Respond with ONLY a valid JSON array of recommendation strings.",
      messages: [
        {
          role: "user",
          content: userPrompt,
