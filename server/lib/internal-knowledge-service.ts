/**
 * Internal Knowledge Vector Service
 * Manages all knowledge storage and retrieval using PostgreSQL with pgvector
 * This replaces Pinecone for cost-effective, scalable knowledge management
 * Supports culinary terms, PDF documents, recipes, and learned knowledge
 */
...
export async function storeInternalKnowledgeVector(
  ...
  try {
    ...
    if (error) {
      console.error("Error storing internal knowledge vector:", error);
      return {
        id: "",
        success: false,
        error: error.message,
    ...
    console.error("Error storing internal knowledge:", errorMsg);
    return {
      id: "",
      success: false,
      error: errorMsg,
...
export async function storeInternalKnowledgeBatch(
  ...
  console.log(
    `[Internal Knowledge] Batch storage complete. Success: ${results.success}, Failed: ${results.failed}`,
  );
...
export async function searchInternalKnowledge(
  ...
  try {
    ...
    if (options.sourceType) {
      filters.source_type = options.sourceType;
    }
    ...
    const results: KnowledgeSearchResult[] = data
      .map((item: any) => {
        const similarity = calculateCosineSimilarity(embedding, item.embedding);
      ...
      .filter((result) => {
        if (options.minConfidence !== undefined) {
          return result.metadata?.confidence >= options.minConfidence;
        }
        return true;
      })
      .slice(0, topK);
    ...
    console.error("Error in internal knowledge search:", error);
    return [];
  }
}
 ...
 */
export async function searchInternalKnowledgeByDomain(
  domain: string,
  limit: number = 20,
): Promise<KnowledgeSearchResult[]> {
  try {
 ...
 */
export async function getKnowledgeBySourceType(
  sourceType: KnowledgeSourceType,
  limit: number = 50,
): Promise<KnowledgeSearchResult[]> {
  try {
    ...
    if (error) {
      console.error("Error fetching by source type:", error);
      return [];
    }
    ...
    console.error("Error getting knowledge by source type:", error);
    return [];
  }
}
 ...
 */
export async function updateInternalKnowledgeVector(
  id: string,
  updates: Partial<Omit<InternalKnowledgeVector, "id" | "embedding">>,
  ...
  try {
    ...
    if (updates.content || updates.title) {
      const textToEmbed = [updates.title, updates.content]
        .filter(Boolean)
        .join(" ");
    ...
    if (error) {
      console.error("Error updating internal knowledge:", error);
      return { success: false, error: error.message };
    }

    console.log(`[Internal Knowledge] Updated vector: ${id}`);

    return { success: true };
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
    console.error("Error updating knowledge:", errorMsg);
...
export async function deleteInternalKnowledgeVector(
  ...
  try {
    ...
    if (error) {
      console.error("Error deleting internal knowledge:", error);
      return { success: false, error: error.message };
    }

    console.log(`[Internal Knowledge] Deleted vector: ${id}`);

    return { success: true };
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
    console.error("Error deleting knowledge:", errorMsg);
    return { success: false, error: errorMsg };
  }
}
 ...
 */
export async function getInternalKnowledgeStats(): Promise<{
  total: number;
...
function calculateCosineSimilarity(vec1: number[], vec2: number[]): number {
  if (vec1.length !== vec2.length) {
    console.warn("Vector lengths do not match for similarity calculation");
    return 0;
  }
 ...
 */
export async function checkInternalKnowledgeHealth(): Promise<{
  available: boolean;
  stats?: {
    total: number;
    bySourceType: Record<string, number>;
