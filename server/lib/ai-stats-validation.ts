 ...
 * Provides rigorous statistical analysis of experimental data
 */
...
interface ReproducibilityScore {
  ...
  consistency_level: "excellent" | "good" | "acceptable" | "poor";
}
...
function detectOutliers(
  ...
  // IQR method (primary)
  const lowerBound = summary.q1 - 1.5 * summary.iqr;
  const upperBound = summary.q3 + 1.5 * summary.iqr;
  ...
  return {
    ...
    percent: (outliers.length / data.length) * 100,
  };
}
...
function calculateCohensD(
  ...
  // Cohen's d
  const d = (results.mean - baseline.mean) / sp;
...
function calculateReproducibility(data: number[]): ReproducibilityScore {
  ...
  if (cv < 5) {
    score = 95;
    interpretation = "Excellent - Less than 5% variation";
    consistency_level = "excellent";
  } else if (cv < 10) {
    score = 85;
    interpretation = "Good - 5-10% variation (typical for food)";
    consistency_level = "good";
  } else if (cv < 20) {
    score = 70;
    interpretation = "Acceptable - 10-20% variation (high for food)";
    consistency_level = "acceptable";
  } else {
    score = 40;
    interpretation = "Poor - Over 20% variation (inconsistent)";
...
export function validateExperimentalResults(
  ...
  // Calculate summaries
  const resultsSummary = calculateSummary(results);
  const baselineSummary = calculateSummary(baseline);

  // Detect outliers
  const outlierAnalysis = detectOutliers(results, resultsSummary);

  // Calculate comparison metrics
  const meanDifference = resultsSummary.mean - baselineSummary.mean;
  const percentDifference = (meanDifference / baselineSummary.mean) * 100;
  ...
  const critical_t = 1.96; // Approximately for large samples
  const ci_margin = critical_t * standardError;

  // Reproducibility
  const reproducibility = calculateReproducibility(results);
  ...
  if (outlierAnalysis.count > 0) {
    confidenceScore -= Math.min(20, outlierAnalysis.count * 8);
  }

  // Reduce confidence for poor reproducibility
  if (reproducibility.score < 60) {
    confidenceScore -= 25;
  }

  // Reduce confidence if sample size is minimal
  if (results.length < 5) {
    confidenceScore -= 10;
  }
