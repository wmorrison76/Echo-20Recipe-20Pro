...
export async function storeRecipeVector(
  ...
  try {
    ...
    if (collaborators && collaborators.length > 0) {
      const { data: vector } = await client
        .from("recipe_vectors")
        .select("id")
      ...
      if (vector) {
        const collaboratorRecords = collaborators.map((collab) => ({
          recipe_vector_id: vector.id,
          collaborator_id: collab,
        }));

        await client
          .from("recipe_vector_collaborators")
          .upsert(collaboratorRecords, {
            onConflict: "recipe_vector_id,collaborator_id",
          });
      }
    }
  } catch (error) {
...
export async function searchSimilarRecipes(
  ...
  try {
    ...
    const { data, error } = await client.rpc("search_similar_recipes", {
      query_embedding: embedding,
      search_organization_id: organizationId,
      search_track: userTrack,
      search_limit: limit,
      include_cross_track: includeCrossTrack,
    });
    ...
    return (data || []).map((match: any) => ({
      ...
      title: match.title,
      track: match.track,
      score: Math.max(0, match.similarity), // Normalize to 0-1
...
export async function getRecipesByTrack(
  ...
  try {
    ...
    const { data, error } = await client
      .from("recipe_vectors")
      .select("id, recipe_id, title, track, metadata")
...
export async function getCrossTrackLearning(
  ...
  try {
    ...
    const { data, error } = await client.rpc("get_cross_track_learning", {
      query_embedding: embedding,
      search_organization_id: organizationId,
      search_limit: limit,
    });
    ...
    return (data || []).map((match: any) => ({
      ...
      title: match.title,
      track: match.track,
      score: Math.max(0, match.similarity),
      metadata: match.metadata,
    }));
  } catch (error) {
...
export async function checkPgvectorHealth(): Promise<{
  ...
  try {
    ...
    const { data, error } = await client.rpc("search_similar_recipes", {
      ...
      search_organization_id: "00000000-0000-0000-0000-000000000000",
      search_track: "fine-dining",
      search_limit: 1,
