/**
 * Pinecone to Internal Knowledge Migration
 * Orchestrates the complete migration of knowledge from Pinecone to internal pgvector storage
 * Handles data transformation, batch processing, and verification
 */

import {
  extractAllPineconeKnowledge,
  extractPineconeKnowledgeBySourceType,
  getPineconeIndexStats,
  countPineconeVectors,
  verifyPineconeConnection,
  transformPineconeToInternalFormat,
  type ExtractionResult,
  type ExtractedPineconeItem,
} from "./pinecone-extraction-service";
import {
  storeInternalKnowledgeBatch,
  getInternalKnowledgeStats,
  type InternalKnowledgeVector,
...
class MigrationController {
  ...
  private currentProgress: MigrationProgress = {
    status: "pending",
    progress: 0,
    message: "Migration ready to start",
  };
  ...
  private updateProgress(update: Partial<MigrationProgress>) {
    ...
    console.log(`[Migration] ${this.currentProgress.message}`);
  }

  /**
   * Run complete migration from Pinecone to internal storage
   */
  async runFullMigration(): Promise<MigrationStats> {
    ...
    try {
      // Step 1: Verify Pinecone connection
      this.updateProgress({
        status: "extracting",
        progress: 5,
      ...
      if (!connectionCheck.connected) {
        throw new Error(`Pinecone connection failed: ${connectionCheck.error}`);
      }

      console.log("[Migration] ✓ Pinecone connection verified");

      // Step 2: Extract Pinecone statistics
      this.updateProgress({
        status: "extracting",
        progress: 10,
        message: "Analyzing Pinecone index...",
      });

      const indexStats = await getPineconeIndexStats();
      const vectorCount = await countPineconeVectors();

      console.log(`[Migration] Found ${vectorCount} vectors in Pinecone`);
      console.log(`[Migration] Index stats:`, indexStats);

      // Step 3: Extract all Pinecone knowledge
      this.updateProgress({
        status: "extracting",
        progress: 15,
        message: `Extracting ${vectorCount} vectors from Pinecone...`,
      });
      ...
      this.updateProgress({
        status: "storing",
        progress: 50,
        message: `Storing ${transformedItems.length} items in internal database...`,
      });
      ...
      for (let i = 0; i < transformedItems.length; i += batchSize) {
        ...
        const totalBatches = Math.ceil(transformedItems.length / batchSize);

        this.updateProgress({
          status: "storing",
          progress: 50 + Math.floor((i / transformedItems.length) * 40),
          message: `Storing batch ${batchNum}/${totalBatches} (${storedCount}/${transformedItems.length} items)...`,
          storedCount,
        });

        const batchResult = await storeInternalKnowledgeBatch(batch, 5);
        storedCount += batchResult.success;
      ...
      this.updateProgress({
        status: "verifying",
        progress: 92,
        message: "Verifying migration success...",
      });

      const internalStats = await getInternalKnowledgeStats();
      console.log("[Migration] Internal knowledge stats:", internalStats);

      // Step 7: Complete
      const endTime = Date.now();
      stats.endTime = new Date().toISOString();
      stats.durationSeconds = Math.floor((endTime - startTime) / 1000);

      this.updateProgress({
        status: "complete",
        progress: 100,
        message: `Migration complete! Stored ${stats.totalStored}/${stats.totalExtracted} items in ${stats.durationSeconds}s`,
        storedCount: stats.totalStored,
  ...
  /**
   * Run selective migration for specific source type
   * Useful for migrating just PDFs, recipes, etc.
   */
  async runSelectiveMigration(sourceType: string): Promise<MigrationStats> {
    if (this.migrationInProgress) {
      throw new Error("Migration already in progress");
    }
    ...
    try {
      this.updateProgress({
        status: "extracting",
        progress: 15,
        message: `Extracting ${sourceType} knowledge from Pinecone...`,
      });
      ...
      this.updateProgress({
        status: "transforming",
        progress: 40,
        message: `Transforming ${extraction.totalItems} items...`,
      });
      ...
      this.updateProgress({
        status: "complete",
        progress: 100,
        message: `Selective migration complete! Migrated ${stats.totalStored} items of type "${sourceType}"`,
        storedCount: stats.totalStored,
        failedCount: stats.totalFailed,
      });
      ...
      this.updateProgress({
        status: "error",
        progress: 0,
        message: `Selective migration failed: ${errorMsg}`,
      });
      throw error;
    } finally {
      this.migrationInProgress = false;
