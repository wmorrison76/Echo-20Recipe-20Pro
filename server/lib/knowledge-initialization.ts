/**
 * Knowledge Initialization Service
 * Automatically initializes and loads all knowledge sources on server startup
 * Ensures Master Dictionary, PDFs, and Pinecone data are available
 */

import { ingestionController } from "./knowledge-ingestion-service";
import { getInternalKnowledgeStats } from "./internal-knowledge-service";

export interface InitializationConfig {
  autoInit: boolean;
  sources: {
    masterDictionary: boolean;
    pinecone: boolean;
  };
}
...
class KnowledgeInitializer {
  ...
  private async _doInitialize(config: InitializationConfig): Promise<void> {
    try {
      console.log("[Knowledge Init] Starting knowledge initialization...");
      ...
      if (config.sources.pinecone) {
        try {
          console.log("[Knowledge Init] Ingesting from Pinecone...");
      ...
      // Final verification
      const finalStats = await getInternalKnowledgeStats();
