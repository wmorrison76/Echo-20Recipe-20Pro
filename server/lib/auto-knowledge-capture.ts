...
/**
 * Automatically capture and store knowledge from an OpenAI-generated recipe
    ...
    if (knowledge.length > 0) {
      for (const item of knowledge) {
        try {
          ...
          console.error("Failed to store knowledge item:", err);
        }
      }
    }
  } catch (error) {
    console.error("Auto-knowledge capture failed:", error);
  }
}
...
async function extractKnowledgeFromRecipe(
  ...
  try {
    const extractionPrompt = `From this generated recipe, extract structured knowledge items that should be stored in Echo's knowledge base:

Recipe Title: ${recipe.title || "Untitled"}
User Request: ${userPrompt}
Service Context: ${serviceContext || "Not specified"}

Recipe Content:
${JSON.stringify(recipe, null, 2)}

Extract the following knowledge items as a JSON array:
...
- confidence: number (0-1, how confident this knowledge is accurate)
...
[
  {
    "type": "recipe",
    "title": "Recipe Name",
    "content": "Full recipe with instructions",
    "domain": "culinary",
    "tags": ["cuisine", "technique", "complexity"],
    "confidence": 0.95,
    "cuisine": "French",
    "complexity": 3
  }
