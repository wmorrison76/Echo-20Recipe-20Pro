 ...
 * Enables pluggable architecture for adding new recipe sources
 */
export interface SiteCrawlerAdapter {
  name: string;
  domain: string;
  language: string;
  region: string;
 ...
 */
export interface FlavorMatrixEntry {
  id: string;
  recipeId: string;
  recipeName: string;
  source: string;
  ...
  techniques: string[];
  sensoryDescriptors: string[]; // "fruity", "nutty", "herbaceous", etc.
  colorProfile?: string; // "golden-brown", "deep caramel", etc.
  textureProfile?: string[]; // "crispy", "creamy", "chunky", etc.
  culturalContext?: string;
  userComments?: string[]; // Raw user flavor notes
  confidence: number; // 0-1, based on data richness
  createdAt: string;
}
...
export interface FlavorMatrixAggregate {
  ...
  ingredientFlavorMap: Record<string, FlavorProfile>;
  techniqueFlavorMap: Record<string, FlavorProfile>;
  sensoryDescriptorFrequency: Record<string, number>;
  culturalFlavorPatterns: Record<string, FlavorProfile[]>;
  updatedAt: string;
}
 ...
 */
export class GlobalCrawlerManager {
  private adapters = new Map<string, SiteCrawlerAdapter>();
  ...
  /**
   * Register a site crawler adapter
   */
   ...
   */
  getAdapter(domain: string): SiteCrawlerAdapter | undefined {
   ...
   */
  getAllAdapters(): SiteCrawlerAdapter[] {
  ...
  /**
   * Crawl recipes from multiple sources with flavor extraction
   */
  async crawlGlobal(
  ...
  /**
   * Update global flavor matrix with new data
   */
  private updateFlavorMatrix(entry: FlavorMatrixEntry): void {
    ...
    for (const ingredient of entry.mainIngredients) {
      if (!this.flavorMatrix.ingredientFlavorMap[ingredient]) {
        this.flavorMatrix.ingredientFlavorMap[ingredient] = entry.flavorProfile;
      } else {
    ...
    for (const technique of entry.techniques) {
      if (!this.flavorMatrix.techniqueFlavorMap[technique]) {
        this.flavorMatrix.techniqueFlavorMap[technique] = entry.flavorProfile;
      } else {
    ...
    for (const descriptor of entry.sensoryDescriptors) {
      this.flavorMatrix.sensoryDescriptorFrequency[descriptor] =
        (this.flavorMatrix.sensoryDescriptorFrequency[descriptor] || 0) + 1;
    }

    // Cultural flavor patterns
    if (entry.culturalContext) {
      if (!this.flavorMatrix.culturalFlavorPatterns[entry.culturalContext]) {
   ...
   */
  getFlavorMatrix(): FlavorMatrixAggregate {
    return this.flavorMatrix;
  }

  /**
   * Get flavor statistics for a specific ingredient
   */
  getIngredientFlavorProfile(ingredient: string): FlavorProfile | null {
    return this.flavorMatrix.ingredientFlavorMap[ingredient] || null;
  }

  /**
   * Get flavor statistics for a specific technique
   */
  getTechniqueFlavorProfile(technique: string): FlavorProfile | null {
    return this.flavorMatrix.techniqueFlavorMap[technique] || null;
  }

  /**
   * Get most common sensory descriptors
   */
...
class RateLimiter {
  ...
  private lastRobotsCheck: number = 0;
  private robotsData: Map<string, string> = new Map();
  ...
  async canMakeRequest(): Promise<boolean> {
    ...
    // Check hour limit
    if (this.requestTimestamps.length >= this.config.requestsPerHour) {
      return false;
    }
