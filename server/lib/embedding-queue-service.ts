...
export class EmbeddingQueueService {
  ...
  constructor(
    ...
    this.workers = Math.max(1, Math.min(workers, 10));
    this.batchSize = Math.max(10, Math.min(batchSize, 200));
    this.retryLimit = Math.max(1, retryLimit);
  }
    ...
    while (this.queue.length > 0 || this.processing.size > 0) {
      ...
      try {
        ...
        batch.forEach((job, idx) => {
          if (embeddings[idx]) {
            this.completed.set(job.itemId, embeddings[idx]);
        ...
        if (this.processing.size < this.workers) {
          this.rateLimitWait = 1000; // Reset on success
        }
      } catch (error) {
  ...
  getCompleted(itemId: string): number[] | undefined {
    return this.completed.get(itemId);
  }

  getAllCompleted(): Map<string, number[]> {
