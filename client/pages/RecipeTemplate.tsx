...
export default function RecipeTemplate() {
  ...
  const recipeFromCtx = useMemo(()=> (id? getRecipeById(id) : undefined), [id, getRecipeById]);
  const [recipe, setRecipe] = useState<any | undefined>(recipeFromCtx);

  useEffect(()=>{ setRecipe(recipeFromCtx); }, [recipeFromCtx]);
  useEffect(()=>{
    if (recipe) return;
    try {
      ...
      if (raw && id) {
        ...
        const found = Array.isArray(list) ? list.find((r:any)=> r.id === id) : undefined;
      ...
      <div className="mx-auto max-w-4xl px-6 py-6 print:px-0">
        ...
        <div className="bg-white rounded-xl border shadow-md p-8 relative">
          ...
          <img
            ...
            style={{ opacity: 0.75, width: 120, height: 'auto', bottom: 6 }}
          />
        </div>
      </div>
    </div>
