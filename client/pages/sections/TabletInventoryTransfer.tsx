...
export default function TabletInventoryTransfer() {
  ...
  const searchItems = useCallback(async (search: string) => {
    ...
    setIsSearching(true);
    try {
      const response = await fetch(`/api/inventory/items?q=${encodeURIComponent(search)}`);
      if (response.ok) {
  ...
  const handleAddTransfer = useCallback(async () => {
    if (!itemSearch.trim() || !quantity || !toDept || !requestedBy) {
      toast({
        ...
        description: "Please fill in item, quantity, destination, and your name",
        variant: "destructive",
      });
      return;
    }
    ...
    const savedTransfers = JSON.parse(localStorage.getItem("tablet:transfers") || "[]");
    ...
    resetForm();
    setShowForm(false);
  }, [itemSearch, quantity, unit, fromDept, toDept, requestedBy, notes, isOnline, toast]);
              ...
              <div className="relative">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    onFocus={() => itemSearch.trim() && setShowSuggestions(true)}
                    className="pl-10 bg-white"
                  />
                  {isSearching && <Loader2 className="absolute right-3 top-3 h-5 w-5 animate-spin text-slate-400" />}
                </div>
