    ...
    <div className="space-y-4">
      ...
      {filtered.length > 0 ? (
        <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-4">
          {filtered.map((img) => (
            <div key={img.id}
              className="mb-4 break-inside-avoid rounded-xl overflow-hidden relative group border bg-card"
              draggable onDragStart={()=>onDragStart(img.id)} onDragOver={(e)=>e.preventDefault()} onDrop={()=>onDropOver(img.id)}
            >
              <button className="absolute top-2 right-2 z-10 rounded-full bg-black/40 p-1 text-white opacity-0 group-hover:opacity-100" onClick={()=>toggleFavorite(img.id)} aria-label="Favorite">
              ...
              <button onClick={()=>openLightboxAt(img.id)} className="block w-full">
                {img.unsupported ? (
                  <div className="h-48 w-full bg-muted flex items-center justify-center text-xs text-muted-foreground">Unsupported preview</div>
                ) : (
                  <img src={img.dataUrl || img.blobUrl} alt={img.name} className="w-full h-auto object-cover" />
                )}
              </button>
              <div className="p-2 flex flex-wrap items-center gap-1 text-[11px]">
