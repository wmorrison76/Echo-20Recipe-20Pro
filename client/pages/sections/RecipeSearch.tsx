      ...
      <div className="grid grid-cols-[120px_1fr] gap-3 p-3 items-start">
        ...
        <div className="prose prose-sm dark:prose-invert max-w-none">
          ...
          <div className="mt-2 flex gap-2">
            ...
            <Button size="sm" variant="outline" onClick={()=>{ try{ const parse=(s:string)=>{ const frac:(Record<string,string>)={"¼":"1/4","½":"1/2","¾":"3/4","⅐":"1/7","⅑":"1/9","⅒":"1/10","⅓":"1/3","⅔":"2/3","⅕":"1/5","⅖":"2/5","⅗":"3/5","⅘":"4/5","⅙":"1/6","⅚":"5/6","⅛":"1/8","⅜":"3/8","⅝":"5/8","⅞":"7/8"}; let t=String(s).replace(/[¼½¾⅐⅑⅒⅓⅔⅕⅖⅗⅘⅙⅚⅛⅜⅝⅞]/g,c=>frac[c]||c); t=t.replace(/(\d)\s*(\d\/\d)/,'$1 $2'); const m=t.match(/^\s*([0-9]+(?:\.[0-9]+)?(?:\s+[0-9]+\/[0-9]+)?)?\s*([a-zA-Z\.]+)?\s*(.*)$/); const qty=m?.[1]?m[1]:''; const unit=m?.[2]?m[2].toUpperCase():''; const rest=(m?.[3]||'').trim(); const [item,...prep]=rest.split(','); return { qty, unit, item:item.trim(), prep:prep.join(',').trim(), yield:'', cost:'' }; }; const draft={ recipeName:r.title, ingredients:(r.ingredients||[]).map((s:string)=>parse(String(s))), directions:Array.isArray(r.instructions)? (r.instructions as any[]).map(String).map((x,i)=>`${i+1}. ${x}`).join('\n') : String((r as any).instructions||''), taxonomy: (r.extra as any)?.taxonomy || undefined }; localStorage.setItem('recipe:draft', JSON.stringify(draft)); }catch{} location.href='/?tab=add-recipe'; }}>Edit</Button>
          </div>
        </div>
      </div>
    </div>
