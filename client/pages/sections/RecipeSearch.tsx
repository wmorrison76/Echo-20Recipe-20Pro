      ...
      <div className="grid grid-cols-[120px_1fr] gap-3 p-3 items-start">
        ...
        <div className="prose prose-sm dark:prose-invert max-w-none">
          ...
          {r.ingredients?.length ? (
            <p className="mt-2 mb-0 text-xs text-muted-foreground line-clamp-2">
              {r.ingredients.join(", ")}
            </p>
          ) : null}
          <div className="mt-2 flex gap-2">
            ...
            <Button size="sm" variant="ghost" onClick={onTrash}>
              Trash
            </Button>
            <a
              href={`/recipe/${r.id}/view`}
...
export default function RecipeSearchSection() {
  ...
  const results = useMemo(() => {
    ...
    switch (cat) {
      case "recent":
        return notDeleted.slice().sort((a, b) => b.createdAt - a.createdAt);
      case "top":
        return notDeleted
          .slice()
          .sort((a, b) => (b.rating || 0) - (a.rating || 0));
      case "favorites":
        return notDeleted.filter((r) => r.favorite);
      case "uncategorized":
        return notDeleted.filter((r) => !r.tags || r.tags.length === 0);
      case "trash":
        return base.filter((r) => !!r.deletedAt);
      default:
        return notDeleted;
    }
  }, [q, searchRecipes, cat]);
  ...
  useEffect(() => {
    ...
    const flag = localStorage.getItem("recipes.demo.loaded.v1");
    if (flag === "1") return;
    (async () => {
      try {
        const sample = [
          {
            ...
            instructions: [
              ...
              "Layer sauce, noodles, ricotta, beef, mozzarella. Repeat.",
              "Top with parmesan and bake 45 min at 375°F.",
          ...
          },
          {
            ...
            instructions: [
              ...
              "Pour coconut milk; simmer 20 minutes.",
              "Season to taste.",
            ],
          },
          {
            ...
            instructions: ["Sear both sides."],
          },
          {
            title: "Green Risotto",
            ingredients: ["rice", "spinach"],
            instructions: ["Cook rice.", "Blend spinach."],
          },
        ];
        const file = new File(
        ...
        localStorage.setItem("recipes.demo.loaded.v1", "1");
      } catch {}
    })();
            ...
            onChange={async (e) => {
              ...
              try {
                ...
                for (let p = 1; p <= doc.numPages; p++) {
                  ...
                  lines.push(
                    ...tc.items.map((i: any) => String(i.str)).filter(Boolean),
                  );
                  lines.push("");
                }
                ...
                const m = joined.match(
                  /(?:ISBN(?:-1[03])?:?\s*)?((?:97[89][- ]?)?\d{1,5}[- ]?\d{1,7}[- ]?\d{1,7}[- ]?[\dX])/i,
                );
                if (m) {
                  ...
                  if (isbn) {
                    try {
                      ...
                      if (res.ok) {
                        ...
                        const file = new File([blob], `book-${isbn}.jpg`, {
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            title="Cards"
            className={`p-2 rounded-md border ${mode === "cards" ? "bg-muted" : ""}`}
          ...
          <button
            ...
            title="Grid 4"
            className={`p-2 rounded-md border ${mode === "grid4" ? "bg-muted" : ""}`}
            aria-label="4-across grid"
          >
          ...
          <button
            ...
            title="List"
            className={`p-2 rounded-md border ${mode === "rows" ? "bg-muted" : ""}`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-[240px_1fr] gap-4 items-start">
              ...
              <div className="text-sm space-y-2">
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  <a
                    className="button border px-3 py-1 rounded"
                    href={`/recipe/${preview.id}/view`}
                  >
                    Open
                  </a>
                  <button
