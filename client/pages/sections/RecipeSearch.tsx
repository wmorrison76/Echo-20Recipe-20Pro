  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 md:px-6 py-4 space-y-4" data-echo-key="page:recipes:search">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Dropzone className="p-4 min-h-[96px] rounded-lg border border-dashed glow self-start" accept=".json,application/json,.docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document,.html,.htm,text/html,.pdf,application/pdf,.xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,.xls,application/vnd.ms-excel,.csv,text/csv,application/zip,application/x-zip-compressed,.zip,image/*" multiple onFiles={onFiles} busy={(total>0 && processed<total) || (bookPhase==='reading' || bookPhase==='importing')} progress={ total>0 ? processed/Math.max(total,1) : (bookPhase==='reading'? (bookTotal? bookPage/Math.max(bookTotal,1): 0) : undefined) }>
        ...
        <div className="rounded-lg border p-2 self-start">
          ...
          <input type="file" accept="application/pdf" onChange={async(e)=>{ const f=e.target.files?.[0]; if(!f) return; if(!confirm('Confirm you own/purchased this cookbook PDF for personal import?')){ (e.target as HTMLInputElement).value=''; return;} try{ setBookFile(f.name); setBookPhase('reading'); setStatus('Reading book PDF...'); const ab = await f.arrayBuffer(); pdfPendingRef.current = f; const pdfjs: any = await import('https://esm.sh/pdfjs-dist@4.7.76/build/pdf.mjs'); const workerSrc='https://esm.sh/pdfjs-dist@4.7.76/build/pdf.worker.mjs'; if(pdfjs.GlobalWorkerOptions) pdfjs.GlobalWorkerOptions.workerSrc=workerSrc; const doc = await pdfjs.getDocument({ data: ab }).promise; setBookTotal(doc.numPages);
            ...
            for(let p=1;p<=doc.numPages;p++){
              ...
              if(hasIng){ candidates.push(p); let guess=''; const top=pageLines.map(normalizeLineA).filter(Boolean).slice(0,10); for(const L of top){ if(/^[A-Z][A-Za-z0-9\-\'\s]{2,80}$/.test(L) || /^([A-Z]\s+){2,}[A-Z][\s:]*$/.test(L)){ guess=L.replace(/\s+/g,' ').trim(); break; } } if(/^see\b/i.test(guess) || /(flexipan|inch|inches|cm|diameter)\b/i.test(guess)) guess=''; setDetected(d=> [...d, { page: p, title: guess || `Candidate p.${p}` }]); }
            }
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="flex justify-between gap-2">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" onClick={async()=>{ try{ setTocOpen(false); const f = pdfPendingRef.current; if(!f) return; setScanOpen(true); setDetectedOpen(true); setDetected([]); setScanPageNo(0); setStatus('Scanning book...'); const ab = await f.arrayBuffer(); const pdfjs: any = await import('https://esm.sh/pdfjs-dist@4.7.76/build/pdf.mjs'); const workerSrc='https://esm.sh/pdfjs-dist@4.7.76/build/pdf.worker.mjs'; if(pdfjs.GlobalWorkerOptions) pdfjs.GlobalWorkerOptions.workerSrc=workerSrc; const doc = await pdfjs.getDocument({ data: ab }).promise; setScanTotal(doc.numPages); const pageTexts:string[]=[]; const normLine=(s:string)=>{ let t=s.replace(/\s+/g,' ').trim(); if (/^([A-Z]\s+){2,}[A-Z](?:\s+\d+)?[\s:]*$/.test(t) && t.length<=60){ t=t.replace(/\s+/g,''); } return t; }; const isLikelyIngredientList=(txt:string)=>{ const ls=txt.split(/\n/).map(s=>s.trim()).filter(Boolean).slice(0,80); const qty=/^(?:\d+(?:\s+\d\/\d)?|\d+\/\d|\d+(?:\.\d+)?|[¼½¾⅓⅔⅛⅜⅝⅞])(?:\s*[a-zA-Z]+)?\b/; let c=0; for(const L of ls){ if(qty.test(L)||/^[•\-*]\s+/.test(L)) c++; } return c>=3; }; const candidates:number[]=[]; for(let p=1;p<=doc.numPages;p++){ const page=await doc.getPage(p); const tc=await page.getTextContent(); const t=(tc.items as any[]).map(i=>String(i.str)).join('\n'); pageTexts.push(t); setScanPageNo(p); const hasIng=/\bingredients?\b/i.test(t)||isLikelyIngredientList(t); if(hasIng){ candidates.push(p); const lines=t.split(/\n/).map(normLine).filter(Boolean); let guess=''; for(let k=0;k<Math.min(lines.length,10);k++){ const L=lines[k]; if(/^[A-Z][A-Za-z0-9\-\'\s]{2,80}$/.test(L) || /^([A-Z]\s+){2,}[A-Z][\s:]*$/.test(L)){ guess=L.replace(/\s+/g,' ').trim(); break; } } setDetected(d=>[...d,{page:p,title:guess||`Candidate p.${p}`}]); } }
                // Learn cookbook terminology
                try { const keepTop=(obj:Record<string,number>,n:number)=> Object.fromEntries(Object.entries(obj).sort((a,b)=>b[1]-a[1]).slice(0,n)); const words:Record<string,number>={}, bigrams:Record<string,number>={}; const textAll=pageTexts.join('\n').toLowerCase().replace(/[^a-z\s]/g,' '); const arr=textAll.split(/\s+/).filter(w=>w.length>=3 && w.length<=24); for(let i=0;i<arr.length;i++){ const w=arr[i]; words[w]=(words[w]||0)+1; if(i<arr.length-1){ const g=`${arr[i]} ${arr[i+1]}`; if(g.length>=5 && g.length<=40) bigrams[g]=(bigrams[g]||0)+1; } } const raw=localStorage.getItem('kb:cook')||'{}'; const kb=JSON.parse(raw); kb.terms = keepTop({...kb.terms, ...words}, 400); kb.bigrams = keepTop({...kb.bigrams, ...bigrams}, 600); kb.books = Array.from(new Set([...(kb.books||[]), f.name.replace(/\.[^.]+$/,'')])); localStorage.setItem('kb:cook', JSON.stringify(kb)); } catch {}
                const starts=candidates.sort((a,b)=>a-b); const items:any[]=[]; for(let i=0;i<starts.length;i++){ const s=starts[i]; const e=(i+1<starts.length? starts[i+1]-1 : pageTexts.length); const textRaw=pageTexts.slice(s-1,e).join('\n'); const text=textRaw.split(/\n/).map(normLine).join('\n'); const lines=text.split(/\n/).map(normLine).filter(Boolean); const lower=lines.map(l=>l.toLowerCase()); const find=(labels:string[])=> lower.findIndex(l=> labels.some(x=> l.startsWith(x))); let ingIdx=find(['ingredients','ingredient']); let instIdx=find(['instructions','directions','method','steps','preparation','procedure']); if(ingIdx<0){ const qty=/^(?:\d+(?:\s+\d\/\d)?|\d+\/\d|\d+(?:\.\d+)?|[¼½¾⅓⅔⅛⅜⅝⅞])(?:\s*[a-zA-Z]+)?\b/; for(let j=0;j<Math.min(lines.length,80);j++){ if(qty.test(lines[j])||/^[•\-*]\s+/.test(lines[j])){ ingIdx=j-1; break; } } } if(instIdx<0&&ingIdx>=0){ for(let j=ingIdx+1;j<Math.min(lines.length,200);j++){ if(/^(instructions|directions|method|steps|preparation|procedure)\b/i.test(lines[j])||/^\d+\.|^Step\s*\d+/i.test(lines[j])){ instIdx=j; break; } } } const getRange=(a:number,b:number)=> lines.slice(a+1,b>a?b:undefined).filter(Boolean); const ings=ingIdx>=0? getRange(ingIdx,instIdx>=0?instIdx:lines.length):[]; const ins=instIdx>=0? getRange(instIdx,lines.length):[]; if((ings.length>=2)||(ins.length>=3)){ let title=''; for(let k=Math.max(0,ingIdx-6); k<Math.min(lines.length, Math.max(ingIdx,8)); k++){ const L=lines[k]||''; if(/^[A-Z][A-Za-z0-9\-\'\s]{2,80}$/.test(L)||/^([A-Z]\s+){2,}[A-Z][\s:]*$/.test(L)){ title=L.replace(/\s+/g,' ').trim(); break; } } if(!title) title=lines[0]||`${f.name.replace(/\.[^.]+$/,'')} p.${s}`; items.push({ title, ingredients: ings, instructions: ins, tags:[f.name.replace(/\.[^.]+$/,'')] }); } }
                if(items.length){ const blob=new Blob([JSON.stringify(items)],{type:'application/json'}); const jf=new File([blob], `${f.name.replace(/\.[^.]+$/,'')}.json`, { type:'application/json' }); const { added } = await addRecipesFromJsonFiles([jf]); setStatus(`Imported ${added} recipes from book.`); } else { setStatus('No recipes detected.'); }
              } catch(e:any){ setStatus(`Failed: ${e?.message||'error'}`);} finally { setScanOpen(false); setDetectedOpen(false); setToc(null); setTocChecked({}); pdfPendingRef.current=null; } }}>Import all (auto)</Button>
