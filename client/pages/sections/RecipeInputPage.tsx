...
const RecipeInputPage = () => {
  ...
  const onGridKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key==='Tab' && !e.shiftKey && col===4 && row===ingredients.length-1) { e.preventDefault(); setIngredients([...ingredients, { qty:'', unit:'', item:'', prep:'', yield:'', cost:'' }]); setTimeout(()=>{ const next=document.querySelector<HTMLInputElement>(`input[data-row="${row+1}"][data-col="0"]`); next?.focus(); },0); return; }
      ...
      <div className="pt-8 h-full overflow-y-auto">
        <div className="w-full px-6 space-y-6 pb-8">
              ...
              <div className="flex flex-col space-y-4">
                ...
                <div className={`text-sm space-y-2 ${isDarkMode ? 'text-cyan-300' : 'text-gray-700'}`}>
                  <div className="flex items-center gap-4">
                    ...
                    <span className="flex items-center gap-1"><span className="font-bold">YIELD:</span><input type="number" value={yieldQty} onChange={(e)=>{yieldManualRef.current=true; setYieldQty(Math.max(0, Number(e.target.value)));}} className={`w-16 px-2 py-1 border rounded text-sm ${isDarkMode ? 'bg-black/50 border-cyan-400/50 text-cyan-300' : 'bg-white border-gray-300'}`} /><input value={yieldUnit} onChange={(e)=>{yieldManualRef.current=true; setYieldUnit(e.target.value.toUpperCase());}} className={`w-14 px-2 py-1 border rounded text-sm uppercase ${isDarkMode ? 'bg-black/50 border-cyan-400/50 text-cyan-300' : 'bg-white border-gray-300'}`} /></span>
          ...
          <div className={`bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-100 ${isDarkMode ? 'bg-gray-800/80 border-gray-700' : ''}`}>
            ...
            <div id="directions-textarea" contentEditable suppressContentEditableWarning ref={dirRef} spellCheck className={`prose prose-sm max-w-none w-full border p-5 rounded-xl shadow-sm transition-all focus:shadow-md focus:ring-2 resize-none min-h-[260px] overflow-y-auto ${isDarkMode ? 'bg-black/50 border-cyan-400/50 text-cyan-300 focus:ring-cyan-400/30' : 'bg-white border-gray-200 text-gray-900 focus:ring-blue-400/30 focus:border-blue-400'}`} style={{ lineHeight:'1.7', whiteSpace:'pre-wrap', fontFamily:selectedFont, fontSize:selectedFontSize }} onKeyDown={(e)=>{ const el=dirRef.current; if(!el) return; const sel=window.getSelection(); if(!sel||!sel.anchorNode) return; if(e.key==='Enter' && !e.shiftKey){ e.preventDefault(); const text=el.textContent||''; let offset=0; const range=sel.getRangeAt(0); const preRange=range.cloneRange(); preRange.selectNodeContents(el); preRange.setEnd(range.endContainer, range.endOffset); const containerText=preRange.toString(); offset=containerText.length; const before=text.slice(0,offset); const after=text.slice(offset); const lineStart=before.lastIndexOf('\n')+1; const currentLine=before.slice(lineStart); const m=currentLine.match(/^\s*(\d+)[\.)]?\s*/); const nextNum=m? String(Number(m[1])+1)+'. ' : '• '; const newText=before+'\n'+nextNum+after; el.textContent=newText; const newOffset=offset+1+nextNum.length; const r=document.createRange(); const walker=document.createTreeWalker(el, NodeFilter.SHOW_TEXT); let seen=0; let node:Node|null=walker.nextNode(); while(node){ const len=(node.textContent||'').length; if(seen+len>=newOffset){ r.setStart(node, newOffset-seen); r.collapse(true); break; } seen+=len; node=walker.nextNode(); } sel.removeAllRanges(); sel.addRange(r); setDirections(newText); }
              }} onInput={(e)=> { let t=(e.target as HTMLDivElement).textContent||''; const fixes:Record<string,string>={ 'fist ':'first ', 'teh ':'the ', 'mized ':'mixed ', 'choped ':'chopped ', 'whiskd ':'whisked ' }; let changed=t; for(const [k,v] of Object.entries(fixes)){ changed=changed.replace(new RegExp('(^|\\s)'+k,'gi'),(s)=> s.toLowerCase().endsWith(k)? s.slice(0,-k.length)+v: s); } if(changed!==t){ (e.target as HTMLDivElement).textContent=changed; const sel=window.getSelection(); if(sel){ const r=document.createRange(); const el=dirRef.current!; r.selectNodeContents(el); r.collapse(false); sel.removeAllRanges(); sel.addRange(r);} } setDirections(changed); }}></div>
          </div>
          ...
          <div className="flex items-center justify-between mt-2">
            <div className="flex items-center gap-2 text-sm">
              ...
              <button onClick={()=>{ const data = serialize(); const url = `${location.origin}${location.pathname}#r=${encodeURIComponent(btoa(JSON.stringify(data)))}`; const body = encodeURIComponent(`${recipeName||'Recipe'}\n${url}`); location.href = `sms:?&body=${body}`; }} className="flex items-center gap-1 text-gray-700 hover:text-black"><Share2 className="w-4 h-4"/>SMS</button>
              <button onClick={()=>{ const html = `<!DOCTYPE html><html><head><meta charset='utf-8'><title>${recipeName||'Recipe'}</title></head><body><h1>${recipeName||'Recipe'}</h1><h3>Ingredients</h3><ul>${ingredients.map(r=>`<li>${[r.qty,r.unit,r.item,r.prep].filter(Boolean).join(' ')}</li>`).join('')}</ul><h3>Directions</h3><pre style="white-space:pre-wrap;font-family:Arial, sans-serif;">${directions}</pre></body></html>`; const blob=new Blob([html],{type:'application/msword'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`${(recipeName||'recipe').replace(/[^a-z0-9-_]+/ig,'_')}.doc`; a.click(); URL.revokeObjectURL(a.href); }} className="flex items-center gap-1 text-gray-700 hover:text-black"><FileDown className="w-4 h-4"/>Word</button>
            </div>
        ...
        onRecipeImport={(data)=>{
          ...
          if (data?.ingredients?.length){
            const rows = (data.ingredients as string[]).map((s:string)=>{
              s = decode(s);
              const fracMap: Record<string,string> = { '¼':'1/4','½':'1/2','¾':'3/4','⅐':'1/7','⅑':'1/9','⅒':'1/10','⅓':'1/3','⅔':'2/3','⅕':'1/5','⅖':'2/5','⅗':'3/5','⅘':'4/5','⅙':'1/6','⅚':'5/6','⅛':'1/8','⅜':'3/8','⅝':'5/8','⅞':'7/8' };
