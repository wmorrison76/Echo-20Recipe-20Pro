          ...
          <div className="rounded-xl border p-3 space-y-2 bg-white/95 dark:bg-zinc-900 ring-1 ring-black/5 dark:ring-sky-500/15">
            ...
            {invTab==='raw' && (
              <>
                ...
                {(()=>{ const q=rawQuery.trim().toLowerCase(); const filtered = q? raw.filter(it=> `${it.name} ${it.category||''} ${it.location||''} ${it.vendor||''} ${it.brand||''}`.toLowerCase().includes(q)) : raw; const pageItems = filtered.slice((rawPage-1)*pageSize, rawPage*pageSize); const total = filtered.reduce((s,it)=> s + ((it.onHand||0) * (it.unitCost||0)), 0); const pageTotal = pageItems.reduce((s,it)=> s + ((it.onHand||0) * (it.unitCost||0)), 0); return (
                  <div className="text-xs text-muted-foreground flex flex-wrap gap-4 mb-1">
                    <span>Total items: {filtered.length}</span>
                    <span>Total value: {fmtCurrency(total)}</span>
                    <span>Page value: {fmtCurrency(pageTotal)}</span>
