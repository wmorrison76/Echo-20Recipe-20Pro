...
export default function RecipeEditor() {
  ...
  const [allergens, setAllergens] = useState<string>(
    String((recipe?.extra as any)?.allergens ?? ""),
  );
  const [cookTime, setCookTime] = useState<string>(
    String((recipe?.extra as any)?.cookTime ?? ""),
  );
  const [cookTemp, setCookTemp] = useState<string>(
    String((recipe?.extra as any)?.cookTemp ?? ""),
  );
              ...
              <div className="aspect-square rounded-lg border bg-muted flex items-center justify-center overflow-hidden">
                {cover ? (
                  ...
                  <div className="text-sm text-muted-foreground p-4 text-center">
                    Drop image in Gallery, then attach below
                  </div>
                )}
              </div>
            </Labeled>
              ...
              <div className="grid grid-cols-3 gap-2 max-h-60 overflow-auto">
                {images.map((img) => (
                  <button
                    key={img.id}
                    onClick={() =>
                      attachImageToRecipeFromGallery(recipe.id, img.name)
                    }
...
function IngredientsTable({ recipeId }: { recipeId: string }) {
  ...
  const rows: Row[] =
    (recipe.extra as any)?.ingredientsTable ??
    Array.from({ length: 10 }, () => ({}));

  const setRow = (idx: number, patch: Partial<Row>) => {
    const next = rows.map((r, i) => (i === idx ? { ...r, ...patch } : r));
    updateRecipe(recipeId, {
      extra: { ...(recipe.extra ?? {}), ingredientsTable: next },
    });
  };
          ...
          {rows.map((r, i) => (
            <tr key={i} className="border-t">
              ...
              <td className="p-1">
                <input
                  value={r.yieldPct ?? ""}
                  onChange={(e) => setRow(i, { yieldPct: e.target.value })}
                  className="w-24 rounded border bg-background px-2 py-1"
                />
              </td>
              <td className="p-1">
