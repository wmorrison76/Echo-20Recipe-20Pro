...
export function AITeamInsights() {
  ...
  const expertiseAreas: ExpertiseArea[] = [
    { domain: 'Molecular Gastronomy', memberCount: 2, successRate: 85, trend: 'up' },
    { domain: 'Fermentation', memberCount: 3, successRate: 81, trend: 'up' },
    { domain: 'Texture Engineering', memberCount: 2, successRate: 78, trend: 'stable' },
    ...
    <div className="w-full space-y-6 p-6 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 rounded-xl border border-rose-500/10">
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-white">Team Insights & Collaboration</h2>
            <p className="text-xs text-slate-400">Expertise map and collaboration opportunities</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-2">
          {teamMembers.map((member, idx) => (
            <div key={idx} className="p-3 rounded-lg bg-slate-800/40 border border-slate-700/30">
              <div className="flex items-start justify-between mb-2">
                <div>
                  <p className="font-medium text-white">{member.name}</p>
                  <p className="text-xs text-slate-400">Active {member.lastActive}</p>
                </div>
                <div className="text-right">
                  <div className="flex items-center gap-1">
                    <Award className="h-3 w-3 text-yellow-400" />
                    <span className="text-sm font-bold text-yellow-300">{member.successRate}%</span>
                  </div>
                  <p className="text-xs text-slate-400">{member.experimentsOwned} experiments</p>
                </div>
              </div>
              <div className="flex flex-wrap gap-1">
                {member.expertise.map((exp, i) => (
                  <span key={i} className="text-xs px-2 py-1 rounded-full bg-slate-700/40 text-slate-300">
                    {exp}
                  </span>
                ))}
              </div>
      ...
      {/* Expertise Areas */}
      <div className="space-y-2">
        <h3 className="text-sm font-semibold text-cyan-300">Expertise Domains</h3>
        <div className="space-y-2">
            ...
            <div key={idx} className="p-3 rounded-lg bg-slate-800/40 border border-slate-700/30">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-cyan-300">{area.successRate}%</span>
                  {area.trend === 'up' && <TrendingUp className="h-4 w-4 text-green-400" />}
                  {area.trend === 'stable' && <Zap className="h-4 w-4 text-slate-400" />}
                </div>
              </div>
              <p className="text-xs text-slate-400">{area.memberCount} team member{area.memberCount !== 1 ? 's' : ''} specialized</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-2">
        <h3 className="text-sm font-semibold text-green-300">Collaboration Opportunities</h3>
        <div className="space-y-2">
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-400 font-semibold">{collab.confidence}%</span>
                </div>
              </div>
            </div>
          ))}
