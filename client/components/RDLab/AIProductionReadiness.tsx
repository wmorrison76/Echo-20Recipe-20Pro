...
import { Loader2, CheckCircle2, AlertTriangle, AlertCircle, Zap } from 'lucide-react';
...
export function AIProductionReadiness() {
  ...
  const readyExperiments = experiments.filter((e) => e.status === 'testing' || e.status === 'ready');
    ...
    <div className="w-full space-y-6 p-6 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 rounded-xl border border-amber-500/10">
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-white">Production Readiness Check</h2>
            <p className="text-xs text-slate-400">Evaluate recipe for production deployment</p>
          </div>
        </div>
      </div>
      ...
      {result && (
        <div className="space-y-4">
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <div className="text-right">
                <p className="text-3xl font-bold text-cyan-300">{result.readinessScore}%</p>
          ...
          {/* Checks */}
          <div className="space-y-2">
            <h3 className="text-sm font-semibold text-cyan-300">Production Checks</h3>
            <div className="space-y-2">
          ...
          {result.documentation.sop && (
            <div className="p-4 rounded-lg bg-slate-800/40 border border-slate-700/30">
              <h4 className="font-semibold text-emerald-300 mb-2">SOP Generated</h4>
              <p className="text-xs text-slate-400 mb-3">Standard Operating Procedure ready for production team</p>
              <div className="bg-slate-950/40 rounded p-3 text-xs text-slate-300 max-h-32 overflow-hidden">
                <p className="line-clamp-4">{result.documentation.sop.substring(0, 200)}...</p>
              </div>
            </div>
          )}
          ...
          <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/20">
            <h4 className="font-semibold text-red-300 mb-2">Allergen Statement</h4>
            <p className="text-sm text-slate-300">{result.documentation.allergenStatement}</p>
          </div>
