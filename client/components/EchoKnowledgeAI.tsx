...
export function EchoKnowledgeAI({
  ...
  return (
    <div
      className={cn(
        "space-y-4 rounded-lg border border-cyan-200 bg-gradient-to-br from-cyan-50 to-blue-50 p-4 dark:border-cyan-900 dark:from-cyan-950 dark:to-blue-950",
        className
      )}
    >
      {/* Header */}
      ...
      <form onSubmit={handleAsk} className="space-y-2">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-md border border-cyan-300 bg-white px-3 py-2 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-cyan-500 dark:border-cyan-700 dark:bg-cyan-950/20"
            disabled={isLoading}
          />
          <Button
            type="submit"
      ...
      {/* Answer Display */}
      {answer && (
        <div className="space-y-2 rounded-md border border-cyan-300 bg-white dark:border-cyan-700 dark:bg-cyan-950/30 p-3">
          {/* Source Badge */}
          {source && (
            <div className="flex items-center gap-2 text-xs">
              ...
              {source === "hybrid" && (
                <span className="flex items-center gap-1 rounded-full bg-cyan-100 dark:bg-cyan-900 px-2 py-0.5 text-cyan-800 dark:text-cyan-100">
      ...
      {showStats && trainingStatus.isTrained && (
        <div className="border-t border-cyan-200 dark:border-cyan-900 pt-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div className="rounded-md bg-cyan-100/50 dark:bg-cyan-950/50 p-2">
              <div className="font-semibold text-cyan-900 dark:text-cyan-100">
                {stats.averageResponseTimeMs}ms
              </div>
              <div className="text-cyan-700 dark:text-cyan-300">Avg Response</div>
            </div>
          </div>
        </div>
      )}
