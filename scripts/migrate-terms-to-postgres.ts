 ...
 * 3. Deduplicates using fuzzy matching
 * 4. Migrates to master_culinary_terms table in Postgres/Supabase
...
async function migrateTermsToPostgres() {
  ...
  try {
    // Load all terms
    console.log("[Migration] Loading terms from in-memory storage...");
    ...
    const allTerms = [...uploadedTerms, ...masterTerms];
    console.log(`[Migration] Loaded ${allTerms.length} total terms`);
    console.log(`  - Master dictionary: ${masterTerms.length}`);
    ...
    // Deduplicate
    console.log("[Migration] Deduplicating terms...");
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      try {
        ...
        if (error) {
          console.warn(
            `[Migration] Batch ${batchNum}/${totalBatches} error:`,
            error.message,
          );
          ...
          const percentComplete = Math.round(
            ((i + batchSize) / records.length) * 100,
          );
          console.log(
            `[Migration] Progress: ${percentComplete}% (${successCount} inserted, ${errorCount} errors)`,
          );
        }
      } catch (error) {
        console.warn(
    ...
    // Verify insertion
    console.log("[Migration] Verifying insertion...");
    const { count, error: countError } = await supabase
      .from("master_culinary_terms")
      .select("*", { count: "exact", head: true });

    if (countError) {
      console.warn("[Migration] Could not verify count:", countError.message);
    } else {
      console.log(`[Migration] Database now contains ${count} terms`);
    }

    // Summary
    console.log("\n[Migration] Migration Complete!");
    console.log(`[Migration] Summary:`);
    console.log(`  Input terms: ${allTerms.length}`);
